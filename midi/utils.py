...
def to_dense(data, dataset_info, device=None):
    ...
    node_features = torch.cat([X, charges], dim=-1)
    pharma_atom = node_features * pharma_mask.unsqueeze(-1)
    ...
    data = PlaceHolder(X=X, charges=charges, pos=pos, E=E, y=y,  node_mask=node_mask, 
                       ...
                       pharma_mask=pharma_mask, pharma_atom=pharma_atom, pharma_atom_pos=pharma_atom_pos)
    return data.mask()


class PlaceHolder:
    def __init__(self, pos, X, charges, E, y, pharma_feat, pharma_coord, 
                 t_int=None, t=None, node_mask=None, pharma_mask=None, pharma_atom=None, pharma_atom_pos=None):
        self.pos = pos
        self.X = X
    ...
    def collapse(self, collapse_charges):
        ...
        copy.pharma_atom = torch.argmax(self.pharma_atom, dim=-1)
        return copy
