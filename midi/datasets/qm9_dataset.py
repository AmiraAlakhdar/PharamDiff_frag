...
from midi.datasets.pharmacophore_utils import mol_to_frag_pharmacophore, FAMILY_MAPPING
...
class QM9Dataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        if self.split == 'train':
            return [f'train_{h}.pt', f'train_n_{h}.pickle', f'train_atom_types_{h}.npy', f'train_bond_types_{h}.npy',
                    ...
                    f'train_angles_{h}.npy', 'train_smiles.pickle', 'qm9_train_smiles_noh.pickle', 'train_frag_data']
        elif self.split == 'val':
            return [f'val_{h}.pt', f'val_n_{h}.pickle', f'val_atom_types_{h}.npy', f'val_bond_types_{h}.npy',
                    ...
                    f'val_angles_{h}.npy', 'val_smiles.pickle', 'qm9_val_smiles_noh.pickle', 'val_frag_data']
        else:
            return [f'test_{h}.pt', f'test_n_{h}.pickle', f'test_atom_types_{h}.npy', f'test_bond_types_{h}.npy',
                    ...
                    f'test_angles_{h}.npy', 'test_smiles.pickle', 'qm9_test_smiles_noh.pickle', 'test_frag_data']
    ...
    def process(self):
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            pharmacophore, frag_dict = mol_to_frag_pharmacophore(mol, name=self.name)
        ...
        final_frag_dict = self.list_to_dict_frag(frag_dicts)
        
        self.save_fragments_to_sdf(final_frag_dict, self.processed_paths[10])
    ...
    def list_to_dict_frag(self, classes_dict_list):
        merged_dict = defaultdict(list)
        for d in classes_dict_list:
    ...
    def save_fragments_to_sdf(self, merged_dict, output_folder):
        os.makedirs(output_folder, exist_ok=True)
        
        for class_name, fragments in merged_dict.items():
        # Create a new SDF writer for each class
            sdf_writer = SDWriter(f"{output_folder}/{class_name}.sdf")
            
            sdf_writer.SetKekulize(False)  # Avoid Kekulization errors
        
            for frag in fragments:
            # Convert each fragment to a RDKit Mol object (assuming fragments are SMILES strings)
                if frag:
                    sdf_writer.write(frag)
        
            # Close the SDF writer after writing all fragments
            sdf_writer.close()  
