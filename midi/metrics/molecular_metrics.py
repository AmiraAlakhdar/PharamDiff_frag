...
from midi.metrics.rdkit_match_eval import match_mol, match_mol_2d
from midi.metrics.pgmg_pharma_match_score import match_score
...
class SamplingMetrics(nn.Module):
    def __init__(self, train_smiles, dataset_infos, test):
        ...
        self.rdkit_match_2d = MeanMetric()
        self.rdkit_valid_pharma_match = MeanMetric()
        self.rdkit_valid_match_2d = MeanMetric()
    ...
    def reset(self):
        for metric in [self.atom_stable, self.mol_stable, self.validity_metric, self.uniqueness,
                       ...
                       self.rdkit_valid_pharma_match, self.pgmg_match_score,
                       self.pgmg_valid_match_score, self.pgmg_percentage, self.pgmg_valid_percentage, 
                       self.rdkit_match_2d, self.rdkit_valid_match_2d]:
            metric.reset()
    ...
    def evaluate(self, generated, local_rank):
        ...
        #pharma_satisfaction = self.conf_pharma_match.compute()
        rdkit_pharma_match = self.rdkit_pharma_match.compute()
        rdkit_valid_pharma_match = self.rdkit_valid_pharma_match.compute()
        rdkit_match_2d = self.rdkit_match_2d.compute()
        rdkit_vaild_match_2d = self.rdkit_valid_match_2d.compute()
        ...
        #print(f"conformer pharmacophore match: {pharma_satisfaction * 100 :.2f}%")
        ...
        print(f"rdkit pharmacophore match 2d: {rdkit_match_2d * 100 :.2f}%")
        print(f"rdkit valid pharmacophore match 2d: {rdkit_vaild_match_2d * 100 :.2f}%")
        ...
        if wandb.run:
            dic = {'Validity': validity,
                   ...
                   'rdkit_pharma_match_2d': rdkit_match_2d,
                   'rdkit_valid_pharma_match': rdkit_valid_pharma_match, 
                   'rdkit_match_valid_2d': rdkit_vaild_match_2d,
    ...
    def rdkit_pharma_satisfaction(self, molecules):
        ...
        for i, mol in enumerate(molecules):
            ...
            if mol.rdkit_mol is None:
                ...
                satisfied_percentage_2d.append(-1)
            else:  
                match_3d = match_mol(mol.rdkit_mol, ref_labels, ref_coords)
                match_2d = match_mol_2d(mol.rdkit_mol, ref_labels, ref_coords)
                satisfied_percentage.append(match_3d)
                satisfied_percentage_2d.append(match_2d)
        
        satisfied_percentage = [i for i in satisfied_percentage if i >= 0]
        satisfied_percentage_2d = [i for i in satisfied_percentage_2d if i >= 0]
        
        #for i in range(len(satisfied_percentage)):
        #    if satisfied_percentage[i] <  0:
        #        satisfied_percentage[i] = 0
        
        
        self.rdkit_pharma_match.update(value= sum(satisfied_percentage) /len(satisfied_percentage),
                                  weight=len(satisfied_percentage))
        self.rdkit_match_2d.update(value= sum(satisfied_percentage_2d) /len(satisfied_percentage_2d),
                                   weight=len(satisfied_percentage_2d))
        # try:
    ...
    def rdkit_valid_pharma_satisfaction(self, valid_molecules, valid_rdkit_mols):
        ...
        try:
            self.rdkit_valid_pharma_match.update(value= sum(valid_match_score) /len(valid_match_score),
                                                weight=len(valid_match_score))
            self.rdkit_valid_match_2d.update(value= sum(satisfied_percentage_2d) /len(satisfied_percentage_2d),
                                             weight=len(satisfied_percentage_2d))
        except:
            self.rdkit_valid_pharma_match.update(value= 0, weight=1)
            self.rdkit_valid_match_2d.update(value= 0, weight=1)
    ...
    def pgmg_match_score_fn(self, molecules):
        ...
        for i, mol in enumerate(molecules):
            ...
            satisfied_percentage.append(match_score(mol.rdkit_mol, np_ref_labels, np_ref_coords))
    ...
    def pgmg_valid_match_score_fn(self, valid_molecules, valid_rdkit_mols):
        ...
        for i, mol in enumerate(valid_molecules):
            ...
            np_ref_labels = ref_labels.detach().cpu().numpy().reshape(-1, 1).astype(int)
            np_ref_coords = ref_coords.detach().cpu().numpy()
            satisfied_percentage.append(match_score(valid_rdkit_mols[i] , np_ref_labels, np_ref_coords))
        valid_match_score = [i for i in satisfied_percentage if i >= 0]
...
def checkPharmacophoreSatisfaction(mol,ref_labels, ref_coords, th=1):
    try:
        mol.rdkit_mol.UpdatePropertyCache(strict=False)
        feat_arr, idx_arr, coord_arr = mol_to_torch_pharmacophore_mol_metrics(mol.rdkit_mol)
    ...
    feat_arr = torch.Tensor(feat_arr).long().to(device)

    try:
        # Calculate the distance matrix using PyTorch operations
        dist_matrix = torch.norm(ref_coords.unsqueeze(1) - coord_arr[None, :, :], dim=2)
    except Exception as e:
        #print(f"An error occurred: {e}")
        return 0

    # Combine both conditions
    matched_indices = torch.where((ref_labels[:, None] == feat_arr) & (dist_matrix <= th))
